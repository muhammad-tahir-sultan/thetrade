...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    // If status is loading, we render the layout framework with skeletons instead of a full-screen spinner
    ...
    return (
        <div className="min-h-screen bg-background text-foreground flex flex-col lg:flex-row overflow-hidden pb-16 lg:pb-0">
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-8">
                            <div className="space-y-3">
                                <div className="h-10 w-48 rounded-xl bg-secondary/5 animate-pulse" />
                                <div className="h-4 w-64 rounded-lg bg-secondary/5 animate-pulse" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="h-48 rounded-[2.5rem] bg-secondary/5 animate-pulse" />
                                <div className="h-48 rounded-[2.5rem] bg-secondary/5 animate-pulse" />
                            </div>
                            <div className="h-96 rounded-3xl bg-secondary/5 animate-pulse" />
                        </div>
                    ) : children}
                </div>
            </main>
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-xl border-t border-secondary/10 px-6 py-3 z-40 flex items-center justify-between">
                {isLoading ? (
                    [1, 2, 3, 4].map((i) => <div key={i} className="w-10 h-10 rounded-xl bg-secondary/5 animate-pulse" />)
                ) : (
                    navItems.map((item) => (
                        <Link
                            key={item.href}
