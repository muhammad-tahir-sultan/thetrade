        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-lg mx-auto pb-24 px-4">
            ...
            {taskRequestStatus === "APPROVED" ? (
                <div className="flex flex-col items-center gap-2 py-2">
                    <GrabButton busy={busy} completed={dailyTasksCompleted} total={maxDailyTasks} onClick={handleGrab} />
                    <p className={cn("text-xs font-medium transition-colors", busy ? "text-amber-500 animate-pulse" : "text-secondary")}>
                        {busy ? "Finding your order..." : "Tap the button to grab your next order"}
                    </p>
                </div>
            ) : (
            ...
            {/* Products grid — 4 columns, all 8 items */}
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-4 gap-2.5">
                    {ITEMS.map((item) => (
                        <button key={item.id} disabled={busy}
                            onClick={() => {
                            ...
                            <div className="w-9 h-9 bg-secondary/5 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                                <item.icon className={cn("w-5 h-5", item.color)} strokeWidth={1.5} />
                            </div>
                            <span className="text-[8px] font-bold text-center leading-tight w-full truncate">{item.name.split(" ")[0]}</span>
                        </button>
                    ))}
                </div>
            </div>
